...
@section('content')
    <div class="content flex flex-col w-full gap-8 py-8 px-10 flex-1">
        <div class="flex gap-8 w-full items-center justify-center">
            <div class="w-2/3">
                <x-display>Edit Pengguna</x-display>
                <x-breadcrumb>
                    Admin &gt; <a href="{{ route('admin.users') }}" class="text-green-700 hover:underline">Kelola Data Pengguna</a> &gt; <span class="text-black">Edit Pengguna</span>
                </x-breadcrumb>
            </div>
            <div class="w-1/3">
        ...
        <div class="flex flex-1 gap-8 w-full">
            <x-card class="w-full flex-col">
                <form action="{{ route('user.update', ['id' => $user->id]) }}" method="POST" class="space-y-6 w-full max-w-lg mx-auto">
                    @csrf
                    <div>
                        <label class="block mb-1 font-semibold text-gray-700">Nama</label>
                        <input value="{{ $user->name }}" name="name" type="text" placeholder="Nama"
                    ...
                    <div>
                        ...
                        <input placeholder="Kosongkan kalau tidak diubah" name="password" type="password" placeholder="Password"
                    ...
                    <div>
                        ...
                        <input value="{{ $user->role }}" name="role" type="text" placeholder="Posisi"
                            class="w-full px-4 py-2 rounded-full bg-gray-100 text-sm placeholder-gray-500 focus:outline-none" required>
                    </div>
                    <div>
                        <label class="block mb-1 font-semibold text-gray-700">No. Telpon</label>
                        <input value="{{ $user->phone }}" name="phone" type="text" placeholder="No. Telpon"
                            class="w-full px-4 py-2 rounded-full bg-gray-100 text-sm placeholder-gray-500 focus:outline-none" required>
                    </div>
