...
@section('content')
    <div class="content flex flex-col w-full gap-8 py-8 px-10 flex-1">
        <div class="flex gap-8 w-full items-center justify-center">
            <div class="w-2/3">
                <x-display>Tambah Tempat Sampah</x-display>
                <x-breadcrumb>
                    Admin &gt; <a href="{{ route('admin.trash_bins') }}" class="text-green-700 hover:underline">Kelola Data
                        Tempat Sampah</a> &gt; <span class="text-black">Tambah Tempat Sampah</span>
                </x-breadcrumb>
            </div>
            <div class="w-1/3">
        ...
        <div class="flex flex-1 gap-8 w-full">
            <x-card class="w-full flex-col">
                <form action="{{ route('trash_bin.store') }}" method="POST" class="space-y-6 w-full max-w-lg mx-auto">
                    @csrf
                    <div>
